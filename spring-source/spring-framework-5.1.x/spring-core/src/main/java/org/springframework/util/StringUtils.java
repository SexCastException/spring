...
public abstract class StringUtils {
	...
	/**
	 * 删除给定 {@code String}中的任何字符。<br>
	 * Delete any character in a given {@code String}.
	 *
	 * @param inString      the original {@code String} 原始字符
	 * @param charsToDelete a set of characters to delete.要删除的字符
	...
	public static String deleteAny(String inString, @Nullable String charsToDelete) {
		...
		for (int i = 0; i < inString.length(); i++) {
			...
			if (charsToDelete.indexOf(c) == -1) {    // 保存charsToDelete中不存在的字符
				sb.append(c);
			}
		}
		return sb.toString();
	...
	public static String[] delimitedListToStringArray(
		...
		// 保存分隔处理后的字符
		List<String> result = new ArrayList<>();
		if (delimiter.isEmpty()) {    // 如果分隔符为空串，则不分隔，只删除指定字符
			...
			if (str.length() > 0 && pos <= str.length()) {    // 对最后一个分隔符索引位置之后的字符串删除charsToDelete指定字符串
				// Add rest of String, but not in case of empty input.
				// 添加其余的非空字符串，即最后一个分隔符之后的非空字符
