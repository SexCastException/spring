...
public class DefaultListableBeanFactory extends AbstractAutowireCapableBeanFactory
	...
	/**
	 * key为beanName，value为 {@link BeanDefinition} <br>
	...
	/**
	 * bean定义名称列表，按注册顺序排列。 <br>
	 * List of bean definition names, in registration order.
	 */
	private volatile List<String> beanDefinitionNames = new ArrayList<>(256);

	/**
	 * 手动注册单例bean的名字，按注册顺序排列 <br>
	...
	/**
	 * 是否包含给定的beanName对应的 {@link BeanDefinition}
	 * ---------------------------------------------------------------------
	...
	/**
	 * 通过指定的beanName获取对应的 {@link BeanDefinition}对象 <br>
	 *
	...
	public void registerBeanDefinition(String beanName, BeanDefinition beanDefinition)
		...
		// existingDefinition不为null，说明该beanName名称的beanDefinition已经被注册过了
		...
		} else {    // 该beanName还没有被注册过
			if (hasBeanCreationStarted()) {
				// Cannot modify startup-time collection elements anymore (for stable iteration)
				synchronized (this.beanDefinitionMap) {
					// 开始注册beanDefinition
					this.beanDefinitionMap.put(beanName, beanDefinition);
					// 将此次注册的beanName和之前已经注册过的beanName合并并保存到updatedDefinitions
	...
	private void updateManualSingletonNames(Consumer<Set<String>> action, Predicate<Set<String>> condition) {
		if (hasBeanCreationStarted()) {
			// Cannot modify startup-time collection elements anymore (for stable iteration)
			synchronized (this.beanDefinitionMap) {
				// 如果manualSingletonNames包含了指定的beanName，则从manualSingletonNames中移除该beanName
